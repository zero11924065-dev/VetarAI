...
<br>当前版本：0.2.1（macOS · Apple Silicon）
...
**⬇️ [下载 VetarAI 0.2.1 安装包](https://github.com/zero11924065-dev/VetarAI/releases/tag/v0.2.1)** **⬇️ [Download VetarAI 0.2.1 Installer](https://github.com/zero11924065-dev/VetarAI/releases/tag/v0.2.1)**
<br>（132MB · macOS Apple Silicon · dmg 格式）

</div>

**如何安装**：下载 `VetarAI-0.2.1-arm64.dmg` → 双击挂载 → 把 VetarAI 拖入「应用程序」文件夹 → 从启动台打开。
> 首装因未签名可能提示"无法验证开发者"：右键 → 打开，或终端执行 `xattr -cr /Applications/VetarAI.app`。
<br>How to Install: Download VetarAI-0.2.1-arm64.dmg → Double-click to mount → Drag VetarAI into the "Applications" folder → Open from Launchpad.
