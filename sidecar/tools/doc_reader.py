"""文档解析：把 .docx/.xlsx/.pptx/.pdf 提取为可读文本 + 格式概要（0.4.19 #4 补 Agent 读文档通道）。

设计取舍（表格必提取 / 格式聚合而非逐段罗列 / 中文字体读 eastAsia / 全部延迟 import / pptx 走 zip 兜底）
与依赖现状（2026-09-10 实测）已迁出：详见 交接/03-修复与调试历史记录.md 第十部分
"""
