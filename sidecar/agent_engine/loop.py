...
def build_system_prompt(
    ...
    base = (
        ...
        "【附件纪律】用户上传的文件已落盘到本机，其**绝对路径**写在用户消息正文里"
        "（形如「（原件已保存：/…）」），文件**内容不会**随消息附上。\n"
        "- 任务需要该文件内容时，⛔ **必须先用 read_file 读取该绝对路径**再作答，"
        "不得声称「看不到文件」「没有收到内容」「无法读取上传的文件」——那是没调用工具，不是能力缺失。\n"
        "- read_file 支持 docx/xlsx/pptx/pdf（自动解析为文本+表格+格式概要）与各类纯文本；"
        "图片附件走视觉输入、无需 read_file。\n"
        "- ⛔ 只有当用户消息里**确实没有**任何路径时，才可如实说明未收到文件。\n"
